...
import {FC, MutableRefObject, PropsWithChildren, Ref, RefObject, useEffect, useRef} from "react";
...
function renderSnow(canvasRef: RefObject<HTMLCanvasElement | undefined>, snowflakes: Snowflake[], density: number, frameRef: MutableRefObject<number | undefined>) {
...
function onCanvasContainerChanged(container: HTMLElement, canvas: HTMLCanvasElement) {
    canvas.width = container.clientWidth;
    canvas.height = container.clientHeight;
}

function snowflakeCountForDimensions(width: number, height: number, density: number) {
...
const SnowflakeProvider: FC<Props> = ({children, pixelsPerFlake}) => {
    ...
    useEffect(function () {
        ...
        const onResize = () => onCanvasContainerChanged(container.current!, canvas.current!);
